...
def response_factory(app, handler):
    ...
    def response(request):
        ...
        #######handler means the function under the URL?? why use "handler(requset)"this format?
...
def datetime_filter(t):
    ...
    dt = datetime.utcfromtimestamp(t).strftime("%d %b %Y  %H:%M")
    return u'%s' % dt
...
def init(loop):
    ...
    add_static(app)
    srv = yield from loop.create_server(app.make_handler(), '52.9.176.79', 9001)
